...
function baseCreateRenderer(
  ...
  // 创建一个ReactiveEffect副作用对象，并执行一次update
  const setupRenderEffect: SetupRenderEffectFn = (
    ...
    const componentUpdateFn = () => {
      if (!instance.isMounted) {
        ...
        // onVnodeMounted
        //如果 VNode 不是异步包装节点，并且 props 中定义了 onVnodeMounted 钩子函数，则继续执行下面的逻辑。
        // 这个逻辑通常用于需要在组件挂载完成后执行某些操作的场景。例如，第三方库的初始化、DOM 操作或者事件监听等。这种设计确保了钩子函数在 DOM 准备好后才被执行，以避免操作未挂载的 DOM 元素。
        if (
          !isAsyncWrapperVNode &&
          (vnodeHook = props && props.onVnodeMounted)
        ) {
          const scopedInitialVNode = initialVNode
          // 排队等待执行 onVnodeMounted 钩子函数：
          // 这是 Vue 3 中的一个函数，用来将一个副作用函数（在此处是 invokeVNodeHook）添加到渲染后的队列中。这些副作用函数会在 DOM 更新完成之后执行。
          queuePostRenderEffect(
        ...
        /**
         * next: 下一个待渲染的 VNode，如果存在，则表示组件需要更新。
          bu: beforeUpdate 钩子函数数组，在组件更新前调用。
          u: updated 钩子函数数组，在组件更新后调用。
          parent: 父组件实例。
          vnode: 当前组件实例的 VNode
         */
        ...
        //禁止在生命周期钩子执行期间的递归更新，防止死循环。
        toggleRecurse(instance, false)
        // 如果 next 存在，则将当前 VNode 的 DOM 元素引用赋值给 next，并调用 updateComponentPreRender 进行 props 和 slots 的更新。
        if (next) {
          next.el = vnode.el
          // 1、更新props和slots
          updateComponentPreRender(instance, next, optimized)
        } else {
          // 如果 next 为空，则将当前 VNode 赋值给 next。 表示没有变化
          next = vnode
        }
        ...
        // 3、获取下个状态的根节点  渲染组件的根节点，返回新的虚拟 DOM 树 nextTree。
        ...
        next.el = nextTree.el
        //如果更新是组件自身触发的（originNext === null），则更新高阶组件 (HOC) 的 VNode 元素引用。
        if (originNext === null) {
...
function getSequence(arr: number[]): number[] {
  ...
  v = result[u - 1]
  while (u-- > 0) {
    // 结果result最长递增子序列元素个数是没有问题，但是位置可能有误（最后一个元素不会有错），重新纠正
    result[u] = v
    v = p[v]
  }
  return result
